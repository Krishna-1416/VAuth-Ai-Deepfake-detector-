...
async def detect_media(file: UploadFile = File(...)):
    ...
    try:
        ...
        if score > 0.7:
            ...
            explanation = "Significant spectral fingerprints and spatial anomalies detected."
        elif score < 0.3:
            prediction = "Real"
            confidence = float(1 - score)
            explanation = "Consistent frequency distribution; no deepfake artifacts found."
        else:
            prediction = "Uncertain"
            confidence = 0.5
            explanation = "Inconclusive results in the frequency domain."
